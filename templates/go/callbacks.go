...
// bdapps callback (inbound webhook) handlers — net/http.
//
...
// ack is the only response bdapps expects.
...
// crash — subscription state above all — publish to a real broker instead.
...
func (c *Callbacks) Register(mux *http.ServeMux) {
	mux.HandleFunc("/api/bdapps/sms/mo", c.MoSMS)
	mux.HandleFunc("/api/bdapps/sms/dlr", c.DeliveryReport)
	mux.HandleFunc("/api/bdapps/ussd", c.USSD)
	mux.HandleFunc("/api/bdapps/subscription/notification", c.SubscriptionNotification)
}

/* ── Shared guards ───────────────────────────────────────────────────────── */

// BdappsSourceIPs restricts callbacks to bdapps's egress addresses. bdapps
...
var BdappsSourceIPs []string

func allowedSource(r *http.Request) bool {
	if len(BdappsSourceIPs) == 0 {
		return true // not configured yet
	}
	ip := strings.TrimSpace(strings.Split(r.Header.Get("X-Forwarded-For"), ",")[0])
	for _, allowed := range BdappsSourceIPs {
		if ip == allowed {
			return true
		}
	}
...
func (c *Callbacks) enqueue(name string, payload map[string]any) {
	...
	default:
		log.Printf("[bdapps] job queue full, dropping %s", name)
	}
}
...
func (c *Callbacks) MoSMS(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[bdapps] mo-sms requestId=%s from=%s",
...
// deliveryStatus normalises the two spellings bdapps and the SMPP gateway use.
...
func (c *Callbacks) DeliveryReport(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[bdapps] delivery-report requestId=%s status=%s",
...
func (c *Callbacks) USSD(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[bdapps] ussd sessionId=%s operation=%s from=%s",
...
func (c *Callbacks) handleUSSDInput(payload map[string]any) {
	...
	send := func(message, operation string) {
		if _, err := c.Client.SendUSSD(ctx, sessionID, source, message, operation); err != nil {
			log.Printf("[bdapps] ussd send failed sessionId=%s: %v", sessionID, err)
		}
	}
	...
	case "1":
		c.Sessions.End(sessionID)
		send("Your balance is Tk. 300.00", "mt-fin")
	case "2":
...
func (c *Callbacks) SubscriptionNotification(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[bdapps] subscription-notification subscriber=%s status=%s",
...
func (c *Callbacks) worker() {
	for next := range c.Jobs {
		...
		default:
			log.Printf("[bdapps] unknown job %s", next.name)
		}
	}
}
...
/* ── Wiring ───────────────────────────────────────────────────────────────
 *
 *   config, err := bdapps.LoadConfig()
 *   if err != nil { log.Fatal(err) }            // fail at boot, not under load
 *
 *   client := bdapps.NewClient(config)
 *   callbacks := bdapps.NewCallbacks(config, client, bdapps.NewMemorySessionStore())
 *
